...
resource "google_artifact_registry_repository" "kluster" {
  location      = "europe-north1"
  repository_id = "kluster"
  format        = "DOCKER"
}
resource "google_artifact_registry_repository_iam_policy" "kluster" {
  location    = google_artifact_registry_repository.kluster.location
  repository  = google_artifact_registry_repository.kluster.name
  policy_data = data.google_iam_policy.ar_kluster.policy_data
}
data "google_iam_policy" "ar_kluster" {}
